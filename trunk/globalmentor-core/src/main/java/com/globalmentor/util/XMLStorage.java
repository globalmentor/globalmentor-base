 ...
 * <p>
 * Special storage/retrieval logic is added for the following classes and types:
 * </p>
 * <ul>
 * <li>arrays</li>
...
public class XMLStorage {
	...
	/**
	 * An empty array of type <code>Object</code>, representing an empty argument list of a method.
	 */
	protected final static Object[] NO_ARGUMENTS = new Object[] {};

	/**
	 * An empty array of type <code>Class</code>, representing an empty parameter list of a method or constructor.
	 */
	//TODO del if not needed	protected final static Class NO_PARAMETERS=new Class[]{};

	/**
	 * The string used to indicate the value <code>null</code>. The character used is the last character in the Unicode private use area, which is guaranteed
	...
	public Document getStorageDocument() {
	 ...
	 */
	public void setStorageDocument(final Document document) {
	...
	*/
	...
	/**
	 * Returns the list of variables, stored in <code>#StoredVariable</code> objects, to be stored and retrieved in the XML document. Currently returns an empty
	 * list. Meant to be overridden by a descendant class to return the appropriate variables.
	...
	*/
	...
	/**
	 * <p>
	 * Retrieves the value represented by <code>name</code> from <code>document</code> using the XPath <code>locationPath</code> and stores it in
	 * <code>object</code> using the object's <code>set<em>Name</em>()</code> function. The value is converted automatically from primitive types, and for all
	 * others a <code>String</code> constructor is expected.
	 * </p>
	 * <p>
	 * If a value for <code>name</code> does not exist, <code>defaultValue</code> will be used. <code>locationPath</code> should be an absolute XPath location
	 * path. <code>object<code> must have a <code>set<em>name</em>()</code> method, and <code>type</code> must have a static <code>valueOf(String)</code>
	 * function.
	 * </p>
	 * @param name The name of the variable from which the <code>set<em>name</em>()</code> method will be derived.
	 ...
	 * @return The actual object retrieved, which will be either the value from the document or, if the value did not exist in the document, the default value.
	 * @throws IOException Thrown if <code>object</code> has no <code>set<em>name</em>()</code> method or <code>type</code> has no <code>valueOf(String)</code>
	 *           method. This is thrown in place of such exceptions as java.lang.reflect.InvocationTargetException and java.lang.NoSuchMethodException. This
	 *           exception is also thrown if the location path is not found, the default value is <code>null</code>, and the appropriate
	 *           <code>set<em>name</em>()</code> method does not accept <code>null</code>.
	 */
	public static Object retrieve(final String name, Class type, final Object object, final Document document, final String locationPath,
		...
		try {
			...
			if(nodeList.size() > 0) { //if we retrieved at least one value from the document
				...
				*/
				...
				//if we couldn't find a value in the document
	...
	/**
	 * Stores the value represented by <code>name</code> from <code>document</code> using the XPath <code>locationPath> and stores it in <code>object</code>. If a
	 * value for <code>name</code> does not exist, <code>defaultValue</code> will be used. <code>locationPath</code> should be an absolute XPath location path.
	 * <code>object<code> must have a <code>set<em>name</em>()</code> method, and <code>type</code> must have a static <code>valueOf(String)</code> function.
	 * @return The actual object retrieved, which will be either the value from the document or, if the value did not exist in the document, the default value.
	 * @throws IOException Thrown if <code>object</code> has no <code>set<em>name</em>()</code> method or <code>type</code> has no <code>valueOf(String)</code>
	 *           method. This is thrown in place of such exceptions as java.lang.reflect.InvocationTargetException and java.lang.NoSuchMethodException; //TODO
	 *           have all these comments been converted?
	 */
	 ...
	 * @param document The XML document in which the object should be stored as the document element.
	 */
	...
	/**
	 * Creates a child element of the given node and stores the given object in the child element. The <code>getXXX()</code> and <code>isXXX()</code> methods are
	 * enumerated and each resulting object is stored.
	 * @param object The object to be stored; must not be <code>null</code>.
	 * @param parentNode The node to use as a parent of the element to be created; must be either a <code>Document</code> or have a valid owner document.
	 */
	...
	/**
	 * Creates a child element of the given node using the specified name and stores the given object in the child element. The <code>getXXX()</code> and
	 * <code>isXXX()</code> methods are enumerated and each resulting object is stored.
	 * @param object The object to be stored, which may or may not be <code>null</code>.
	 * @param parentNode The node to use as a parent of the element to be created; must be either a <code>Document</code> or have a valid owner document.
	 * @param elementName The name of the element to create in which the object will be stored.
	 */
	...
	/**
	 * Stores the given object in the specified element. The <code>getXXX()</code> and <code>isXXX()</code> methods are enumerated and each resulting object is
	 * stored. Only array properties and properties with corresponding <code>setXXX()</code> methods for objects that have default constructors are stored.
	 * @param object The object to be stored, which may or may not be <code>null</code>.
				...
				for(int i = 0; i < methods.length; ++i) { //look at each of the methods passed to us
					...
					if(Modifier.isPublic(modifiers) && method.getParameterTypes().length == 0) { //if this is a public method with no parameters
						...
						if(methodName.startsWith(GET)) //if this is a getXXX() method
							...
							//if this method name starts with neither "get" nor "is"
	...
	/**
	 * Retrieves an object of the given type from the XML document stored in the input stream.
	 ...
	 * @throws InstantiationException Thrown if an instance of the given object type could not be created.
	 * @throws IllegalAccessException Thrown if the application cannot access the given object type.
	 * @throws InvocationTargetException Thrown if the object constructor throws an exception.
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	public static Object retrieve(final InputStream inputStream, final Class type) throws FileNotFoundException, IOException, InstantiationException,
	 ...
	 * Retrieves an object of the given type from the XML document stored in the input stream.
	 * @param inputStream The source of the XML document in which the object is stored.
	 * @param sourceObject An object (such as a <code>URL</code> or <code>File</code) representing the source of the document.
	 * @param type The type of object stored in the XML document.
	 * @throws IOException Thrown if there is an error reading from the input stream.
	 * @throws InstantiationException Thrown if an instance of the given object type could not be created.
	 * @throws IllegalAccessException Thrown if the application cannot access the given object type.
	 ...
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	protected static Object retrieve(final InputStream inputStream, final Object sourceObject, final Class type) throws IOException, InstantiationException,
			IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	...
	/**
	 * Retrieves an object of the given type from the XML document stored in the file.
	 ...
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	public static Object retrieve(final File file, final Class type) throws FileNotFoundException, IOException, InstantiationException, IllegalAccessException,
	 ...
	 */
	public static Object retrieve(final File file, final Class type, final boolean useBackup) throws FileNotFoundException, IOException, InstantiationException,
			IllegalAccessException, InvocationTargetException, NoSuchMethodException {
	...
	/**
	 * Stores the given object to the file as an XML document. A temporary file is used and then copied to ensure error do not affect the origina file. If a
	 * backup is created, its filename is formed by adding a ".backup" extension to the filename.
	 * @param object The object to store.
	 * @param file The file in which the object should be stored.
	...
	/**
	 * Constructs an instance of the specified class from the information stored in the given XML document.
	 ...
	 * @throws IllegalAccessException Thrown if the class or initializer of the object is not accessible.
	 * @throws InstantiationException Thrown if the class represents an abstract class, an interface, an array class, a primitive type, or void; or if the
	 *           instantiation fails for some other reason.
	 * @throws InvocationTargetException Thrown if the object constructor throws an exception.
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	public static Object retrieve(final Document document, final Class objectClass) throws IllegalAccessException, InstantiationException,
			InvocationTargetException, NoSuchMethodException {
		return retrieve(document.getDocumentElement(), objectClass); //construct and return the object from the root element
	}

	/**
	 * Constructs an instance of the specified class from the information stored in the given element.
	 * @param element The element that holds the information about the object.
	 * @param objectClass The class for which an instance should be created.
	 * @return An instance of the object class constructed from the given element; <code>null</code> may be returned if the stored object value was
	 *         <code>null</code>.
	 * @throws IllegalAccessException Thrown if the class or initializer of the object is not accessible.
	 * @throws InstantiationException Thrown if the class represents an abstract class, an interface, an array class, a primitive type, or void; or if the
	 *           instantiation fails for some other reason.
	 ...
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	protected static Object retrieve(final Element element, final Class objectClass) throws IllegalAccessException, InstantiationException,
			InvocationTargetException, NoSuchMethodException {
	...
	/**
	 * Creates an instance of the specified type, using information in the given element if needed. In most cases and for most JavaBeans, the default constructor
	 * is called and information in the element is ignored. Some legacy classes, such as <code>java.awt.Dimension</code>, do not require appropriate
	 * <code>setXXX()</code> methods and require a special constructor to be called with information from the elemenet.
	 * @param element The element which contains initialization information for the object.
	 ...
	 * @throws NoSuchMethodException Thrown if an initialization method, if needed, could not be found for the class.
	 */
	protected static Object createObject(final Element element, final Class type) throws IllegalAccessException, InstantiationException,
			InvocationTargetException, NoSuchMethodException {
		...
		if(type.equals(java.awt.Color.class)) { //if this is a Color
			...
			return createObject(element, type, new Class[] { Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE },
					new String[] { "Red", "Green", "Blue", "Alpha" }); //construct a Color from its red, green, blue, and alpha properties
			...
			//if this class doesn't need special initialization
			return type.newInstance(); //for all other classes, create a new instance of the class, using its default constructor
	}

	/**
	 * Creates an instance of the specified type, using an array of property names for its constructor, getting information from the given element. A
	 * <code>java.awt.Color</code> object, for example, would be created with property names of <code>{"Red", "Green", "Blue", "Alpha"}</code>. Each of these
	 * properties must have previously been stored in the element.
	 * @param element The element which contains initialization information for the object.
	 * @param type The type of class to create.
	 * @param constructorParameterTypes The types of parameters used by the constructor.
	 * @param constructorPropertyNames An array of property names, capitalized as appropriate for a <code>set</code> metehod, for initializing the object in a
	 *          constructor.
	 * @return An object created from the appropriate constructor.
	 * @throws IllegalAccessException Thrown if the constructor could not be accessed.
	 * @throws InstantiationException Thrown if there was an error creating the object.
	 * @throws InvocationTargetException Thrown if the constructor throws an exception.
	 * @throws NoSuchMethodException Thrown if the given constructor could not be found for the class.
	 */
	protected static Object createObject(final Element element, final Class type, final Class[] constructorParameterTypes, final String[] constructorPropertyNames)
	...
	/**
	 * Creates an instance of the specified type, using the default constructor and then calling a method with certain properties as arguments, getting
	 * information from the given element. Each of these properties must have previously been stored in the element.
	 * @param element The element which contains initialization information for the object.
	 * @param type The type of class to create. The class must have a default constructor.
	 ...
	 * @param methodPropertyNames An array of property names, capitalized as appropriate for a <code>set</code> method, for initializing the object in a
	 *          constructor.
	 * @return An object created and initialized from the appropriate method.
	 * @throws IllegalAccessException Thrown if the constructor could not be accessed.
	 * @throws InstantiationException Thrown if there was an error creating the object.
	 * @throws InvocationTargetException Thrown if the constructor throws an exception.
	 * @throws NoSuchMethodException Thrown if the given method could not be found for the class.
	 */
	protected static Object createObject(final Element element, final Class type, final String methodName, final Class[] methodParameterTypes,
			final String[] methodPropertyNames) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException {
		...
		*/
		throw new InstantiationException("Could not find an appropriate initialization method \"" + methodName + "\" for " + type.getName()
				+ " for the properties " + methodPropertyNames + "."); //show that we couldn't find an appropriate method
	}

	/**
	 * Retrieves a property value from the specified element with the appropriate name, assuming the specified type. Currently only attributes are searched for
	 * property values. TODO fix for child elements, if needed
	 * @param element The element that holds the information about the object.
	 * @param propertyName The name of the property, capitalized appropriately for a <code>get</code> method.
	 * @param propertyType The type of object to create.
	 * @return An object representing the value of the property, created from the information in the element; this value may be <code>null</code>, based upon the
	 *         value stored in the element.
	 * @throws InstantiationException Thrown if a value could not be created for the property from the information in the element.
	 */
	...
	/**
	 * Determines the storage name for the given class. This name is used for the names of elements in the XML document. The name is determined for objects by
	 * removing the package name, if present, and the name of any enclosing class (for internal classes), and decapitalizing the first letter. Names for arrays
	 * are created by adding an "s" or "es" to the name of the type of object they store, after the name of the object has been processed as above.
	...
	protected static String getStorageName(final Class objectClass) {
		if(objectClass.isArray()) { //if this is an array object
			...
			//if this isn't an array object, the element name will be the decapitalized version of the simple class name
			return Java.getVariableName(Classes.getSimpleName(objectClass)); //get the decapiatlized version of the simple class name
	}

	/**
	 * Finds or constructs an object of the specified type that represents the given string value. If the type is one of the primitive types, an appropriate
	 * primitive wrapper class is constructed. Otherwise, a string constructor for the type is searched. Values for string types are simply returned as-is.
	 * @param value The string value to be converted to an object of the appropriate type.
	 * @param type The type of object which should represent the given string value.
	 * @return An object of the appropriate type representing the given string value, which may be null, based upon the value.
	 * @throws InstantiationException Thrown if the object cannot be created for some reason.
	 * @see NULL_STRING
	 */
	...
	/**
	 * Determines whether an object that has a particular class should be stored as a string value (i.e. using the object's <code>toString()</code> method) in an
	 * attribute instead of as a child element. These classes include:
	 * <ul>
	...
	protected static boolean isStringStorage(final Class type) {
		return ((type.isPrimitive() && !type.equals(Void.TYPE)) //if this is a primitive type besides void
				|| type.equals(java.io.File.class) || type.equals(java.lang.String.class) || type.equals(java.net.URL.class)) ? true : false; //return true if the class is one of our recognized types for string storage
	}
	...
	protected static boolean isStorable(final Class objectType) {
		return objectType.isArray() //if this is an array
				...
				//  meaning we can reconstruct it (this correctly leaves out getClass()),
				|| Classes.getPublicDefaultConstructor(objectType) != null || objectType.equals(java.awt.Color.class); //or if this is a Color
	}

	/**
	 * Determines whether an object's property should be stored.
	 * @param objectType The type of object being stored.
	 * @param propertyName The name (suitable for concatenation with "set") of the property to be stored.
	...
	/**
	 * The encapsulation of the information needed to store and retrieve a variable from an XML document. If a particular XPath location path is invalid for a
	 * particular document, the default value object will be used. If the default value is <code>null</code>, that value will be used if the
	 * <code>set<em>name</em>()</code> method supports it. If the method does not support <code>null</code> (it is a primitive value, for example), an
	 * <code>IOException</code> will be thrown.
	 */
	public static class StoredVariable {

		/**
		 * The name of the variable. This is conventionally in uppercase, as the appropriate object's code>get<em>name</em>()</code> and code>set<em>name</em>
		 * ()</code> methods will be used to store and retrieve the value.
		 */
		public String name;
		 ...
		 * @param newLocationPath The XPath location path of the value's location in the document.
