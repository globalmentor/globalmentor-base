  ...
  // Bulk Modification Operations
	...
	*/
	public boolean addAll(final int index, final Collection<? extends E> collection)
	{
		...
		{
			final int size=size();	//get the size of this list
			Conditions.checkIndexBounds(index, size);	//check the bounds of the index
	...
	*/
	public E remove(final int index)
	{
		...
		try
		{
			...
			Conditions.checkIndexBounds(index, size);	//make sure the index is within the correct bounds (we don't need to use our local routine because we've already calculated the size
