...
public class CharacterEncoding //TODO remove BOM storage, and then replace class with java.nio.Charset
{
	...
	/**
	 * The non-<code>null</code> array of byte order mark bytes, if any, that were used to determine this character encoding.
	 */
	...
	/**
	 * Returns the canonical string representation of the character encoding UTF-16 little endian, for example, would return "utf-16le". The canonical string
	...
	/**
	 * Creates a new character encoding from an encoding string. The character encoding will be given the canonical byte order mark for this family and
	 * endianness, if appropriate.
	 ...
	 * @param encoding The encoding string, specifying big or little-endian if appropriate.
	 * @param bom The non-<code>null</code> array of byte order mark bytes, if any, that were used to determine this character encoding.
	...
	/**
	 * Creates a new character encoding with a family and byte order. The character encoding will be given the canonical byte order mark for this family and
	 * endianness, if appropriate.
	 * @param family The encoding family, one of the constants <code>UTF8</code>, <code>UTF16</code>, or <coode>UCS4</code>.
	 * @param endian Whether the character encoding is little-endian or big-endian, or <code>null</code> if endianness is not specified.
	...
	/**
	 * Determines the canonical byte order mark for representing this character encoding. For UTF-16 and UCS-4, if no byte order is present then little-endian is
	 * assumed.
	 * @param family The encoding family, such as one of the constants <code>UTF_8</code>, <code>UTF_16</code>, or <coode>UCS4</code>.
	 * @param endian Whether the character encoding is little-endian or big-endian, or <code>null</code> if endianness is not specified.
	 * @return The canonical byte order mark for representing this family and byte order, which can be an empty array if there is no preferred byte order mark for
	 *         this encoding.
	...
	public int getBytesPerCharacter() {
		...
		if(CharacterEncoding.UTF_8.equalsIgnoreCase(family)) //if the encoding family is UTF-8
			...
			//there should be no other encoding families, but just in case
			return 1; //the default is one byte per character
	}

	/**
	 * Determines if the byte order mark of this character encoding corresponds to the specified byte order mark.
	 * @param bom The expected byte order mark.
	 * @return <code>true</code> if the first bytes of the byte order mark used to determine this character encoding correspond to the specified byte order mark
	 *         bytes, else <code>false</code>.
	 */
	 ...
	 * @param bom The expected byte order mark.
	 * @return <code>true</code> if the first bytes of the given byte array correspond to the specified byte order mark bytes, else <code>false</code>.
	 */
	...
	/**
	 * Creates a <code>CharacterEncoding</code> object from the given Byte Order Mark. If no valid Byte Order Mark is present, <code>null</code> is returned.
	 * @param byteOrderMarkArray The array of bytes representing the Byte Order Mark.
	 * @return The character encoding object from the given Byte Order Mark, or <code>null</code> if no Byte Order Mark is present.
	 */
	public static CharacterEncoding create(final byte[] byteOrderMarkArray) {
		if(isByteOrderMark(byteOrderMarkArray, CharacterEncoding.BOM_UTF_16_BIG_ENDIAN)) //FE FF: UTF-16, big endian
			...
			//if we don't recognize the byte order mark
			return null; //show that we didn't find a byte order mark, and thus cannot construct a character encoding object
	}

	/**
	 * Creates a <code>CharacterEncoding</code> object from the given Byte Order Mark or, if a true Byte Order Mark is not present, by comparing the bytes to
	 * expected characters.
	 * @param byteOrderMarkArray The array of bytes representing the Byte Order Mark.
	...
	public static CharacterEncoding create(final byte[] byteOrderMarkArray, final String expectedCharacters) { //TODO maybe allow a default eight-bit encoding to be specified
		...
		if(characterEncoding == null) { //if no byte order mark was found, try to compare characters with what is expected
			if(byteOrderMarkArray.length >= 4 && expectedCharacters.length() >= 1) { //if there are at least four bytes in the array, and we have at least one character to compare them with
				...
				if(byteOrderMarkArray[0] == 0x00 && byteOrderMarkArray[1] == 0x00 && byteOrderMarkArray[2] == 0x00 && byteOrderMarkArray[3] == (byte)firstExpectedChar) //00 00 00 X1: UCS-4, big-endian (1234 order)
				...
				if(byteOrderMarkArray[0] == (byte)firstExpectedChar && byteOrderMarkArray[1] == 0x00 && byteOrderMarkArray[2] == 0x00 && byteOrderMarkArray[3] == 0x00) //X1 00 00 00: UCS-4, little-endian (4321 order)
				...
				*/
				if(expectedCharacters.length() >= 2) { //if we have at least two character with which to compare the bytes in the array
					...
					if(byteOrderMarkArray[0] == 0x00 && byteOrderMarkArray[1] == (byte)firstExpectedChar && byteOrderMarkArray[2] == 0x00
							&& byteOrderMarkArray[3] == (byte)secondExpectedChar) //00 X1 00 X2: UTF-16, big-endian, no Byte Order Mark
					...
					if(byteOrderMarkArray[0] == (byte)firstExpectedChar && byteOrderMarkArray[1] == 0x00 && byteOrderMarkArray[2] == (byte)secondExpectedChar
							&& byteOrderMarkArray[3] == 0x00) //X1 00 X2 00: UTF-16, little-endian, no Byte Order Mark
