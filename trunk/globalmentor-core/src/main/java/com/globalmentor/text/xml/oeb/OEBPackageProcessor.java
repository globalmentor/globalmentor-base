...
/**
 * Class which parses an input stream containing an OEB publication. If the input stream contains an OEB 1.x package document, it will be converted to an
 * XEbook.
 */
public class OEBPackageProcessor {
	...
	/**
	 * Reads an OEB publication from a package file and converts it to an RDF data model. If the input stream contains an OEB 1.x package document, it will be
	 * converted to an XEbook.
	 ...
	 * @throws IOException Thrown if there is an error reading the package information from the input stream.
	 */
	 ...
	 * converted to an XEbook.
	 * @param packageInputStream The input stream containing the package information.
	 * @param packageURI The URI to the package information, or <code>null</code> if the package URI is not available.
	 * @return The RDF data model of the package information.
	 * @throws IOException Thrown if there is an error reading the package information from the input stream.
	 */
	//TODO add error reporting to this code
	 ...
	 * @param rdf The RDF data model to use, which should already have the appropriate resource factories registered.
	 * @param oeb1PackageDocument The XML document containing the OEB 1.x package information.
	...
	protected RDF convertOEB1Package(final RDF rdf, final Document oeb1PackageDocument) throws URISyntaxException {
		...
		final List<Node> dcIdentifierElementList = (List<Node>)XPath.evaluatePathExpression(rootElement, XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_METADATA
				+ XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_METADATA_DC_METADATA + XPath.LOCATION_STEP_SEPARATOR_CHAR
				+ PKG_ELEMENT_MANIFEST_DC_METADATA_DC_IDENTIFIER);
		for(int i = 0; i < dcIdentifierElementList.size(); ++i) { //look at each DC metadata element
			...
			if("uri".equals(scheme)
					|| URN_SCHEME.equals(scheme) //TODO del "uri" scheme if not needed
					|| dcIdentifierElementText.startsWith(URN_SCHEME + SCHEME_SEPARATOR) || dcIdentifierElementText.startsWith(HTTP_URI_SCHEME + SCHEME_SEPARATOR)
					|| dcIdentifierElementText.startsWith(HTTPS_URI_SCHEME + SCHEME_SEPARATOR) || dcIdentifierElementText.startsWith(FILE_SCHEME + SCHEME_SEPARATOR)) {
		...
		final List<Node> dcMetadataElementList = (List<Node>)XPath.evaluatePathExpression(rootElement, XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_METADATA
				+ XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_METADATA_DC_METADATA + XPath.LOCATION_STEP_SEPARATOR_CHAR + XPath.WILDCARD_CHAR);
		...
		//XPath: /manifest/item
		final List<Node> manifestElementList = (List<Node>)XPath.evaluatePathExpression(rootElement, XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_MANIFEST
		...
		for(int i = 0; i < manifestElementList.size(); ++i) { //look at each manifest element
			...
			final ContentType itemMediaType = ContentType.create(itemElement.getAttributeNS(null, PKG_MANIFEST_ITEM_ATTRIBUTE_MEDIA_TYPE)); //get the item's media type
		...
		*/
		...
		//XPath: /spine/itemref
		final List<Node> spineElementList = (List<Node>)XPath.evaluatePathExpression(rootElement, XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_SPINE
		...
		final List<Node> guideElementList = (List<Node>)XPath.evaluatePathExpression(rootElement, XPath.LOCATION_STEP_SEPARATOR_CHAR + PKG_ELEMENT_GUIDE
