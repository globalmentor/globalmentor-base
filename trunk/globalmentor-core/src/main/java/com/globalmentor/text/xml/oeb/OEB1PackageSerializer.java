...
public class OEB1PackageSerializer {
	...
	/**
	 * Serializes the package for the specified publication to the given output stream using the UTF-8 encoding with the UTF-8 byte order mark.
	...
	/**
	 * Serializes the package for the specified publication to the given output stream using the specified encoding. Any byte order mark specified in the
	 * character encoding will be written to the stream.
	 * @param publication The OEB publication to serialize.
	 * @param outputStream The stream into which the document should be serialized.
	 ...
	 */
	public static void serialize(final OEBPublication publication, final OutputStream outputStream, final Charset charset) throws IOException,
	...
	public static Document generatePackage(final OEBPublication publication) {
		...
		dcMetadataElement.setAttributeNS(XML.XMLNS_NAMESPACE_URI.toString(), XML.createQName(XML.XMLNS_NAMESPACE_PREFIX, DCMI_ELEMENTS_NAMESPACE_PREFIX),
				DCMI10_ELEMENTS_NAMESPACE_URI.toString());
		//add the attribute, xmlns:oebpackage="http://openebook.org/namespaces/oeb-package/1.0/"
		dcMetadataElement.setAttributeNS(XML.XMLNS_NAMESPACE_URI.toString(), XML.createQName(XML.XMLNS_NAMESPACE_PREFIX, OEB1_PACKAGE_NAMESPACE_PREFIX),
				OEB1_PACKAGE_NAMESPACE_URI.toString());
		...
		while(propertyIterator.hasNext()) { //while there are more properties
			...
			if(DCMI11_ELEMENTS_NAMESPACE_URI.equals(propertyNamespaceURI) || DCMI10_ELEMENTS_NAMESPACE_URI.equals(propertyNamespaceURI)) {
				...
				if(propertyValueObject instanceof RDFLiteral) { //if the value is a literal
					...
					if(TITLE_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_TITLE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Creator>
					else if(CREATOR_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_CREATOR, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Subject>
					else if(SUBJECT_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_SUBJECT, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Description>
					else if(DESCRIPTION_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_DESCRIPTION, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Publisher>
					else if(PUBLISHER_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_PUBLISHER, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Contributor>
					else if(CONTRIBUTOR_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_CONTRIBUTOR, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Date>
					else if(DATE_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_DATE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Type>
					else if(TYPE_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_TYPE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Format>
					else if(FORMAT_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_FORMAT, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					...
					//<package><metadata><dc-metadata><dc:Source>
						...
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_SOURCE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Language>
					else if(LANGUAGE_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_LANGUAGE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Relation>
					else if(RELATION_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_RELATION, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Coverage>
					else if(COVERAGE_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_COVERAGE, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
					//<package><metadata><dc-metadata><dc:Rights>
					else if(RIGHTS_PROPERTY_NAME.equals(propertyLocalName)) {
						XML.appendElementNS(dcMetadataElement, DCMI10_ELEMENTS_NAMESPACE_URI.toString(), PKG_ELEMENT_MANIFEST_DC_METADATA_DC_RIGHTS, propertyValue); //add the OEBPPS 1.x DC metadata element
					}
				}
			}
		}
	 ...
	 * @param publicationURI The URI of the publication, or <code>null</code> if the publication URI has no bearing on identifiers used.
	...
	/**
	 * Creates an ID for an item that conforms to the production of an XML name. If a URI is provided for the publication, the local name is separated from that
	 * URI (for serializing RDF-based models, for instance). Otherwise, all XML non-name characters are converted to an underscore ('_'); if the first letter of
	 * the string is not correct for an XML name, the string will be prevised with 'x'.
	...
	public static String createItemID(final String identifier, final URI publicationURI) {
		if(!XML.isName(identifier)) { //if the identifier is not a name already
			...
			for(int i = stringBuffer.length() - 1; i >= 0; --i) { //look at each character from right to left
				if(!XML.isNameChar(stringBuffer.charAt(i))) { //if this is not a name character
					...
					if(stringBuffer.charAt(i) == '#' && i > 0) //if this is a fragment character that isn't the first characterTODO use a constant
						...
						//if this is some other sort of non-name character
			...
			//if the identifier is already a name
