...
public class Numbers {
	...
	/**
	 * Returns a big decimal representation of the given number. If the number is already a {@link BigDecimal}, the same number object is returned.
	 ...
	 * @return The value <code>0</code> if the first number is numerically equal to the second number; a value less than <code>0</code> if the first number is
	 *         numerically less than the second number; or a value greater than <code>0</code> if the first number is numerically greater than the second number.
	...
	/**
	 * Compares the two specified objects, one of which must be an instance of {@link Number}, and either of which may be <code>null</code>. Numbers are sorted
	 * before non-numbers, and two numbers are compared.
	...
	/**
	 * Compares the two specified objects, one of which must be an instance of {@link Number}, and either of which may be <code>null</code>. Numbers are sorted
	 * before non-numbers, and two numbers are compared. If ambiguous sorting is requested, if neither object is a number the objects will be considered equal;
