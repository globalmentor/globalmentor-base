...
public abstract class AbstractCache<K, Q extends AbstractCache.Query<K>, V, D extends Cache.Data<V>> implements Cache<Q, V> {
	...
	/**
	 * Sets whether fetching new values is synchronous. Changing this value does not change the sychronicity of fetches that have already started but have not yet
	 * completed.
	...
	private final ReadWriteLockCollectionMap<K, CacheFetchListener<Q, V>, List<CacheFetchListener<Q, V>>> cacheFetchListenerMap = new DecoratorReadWriteLockCollectionMap<K, CacheFetchListener<Q, V>, List<CacheFetchListener<Q, V>>>(
			new ArrayListHashMap<K, CacheFetchListener<Q, V>>());
	...
	/**
	 * Determined if the given data is still cached for the given query. This method provides a way to tell if previously retrieved data is still valid.
	 * @param query The query for requesting a value from the cache.
	...
	*/
	...
	/**
	 * Determines if a given cached value is stale. This version checks to see if the age of the cached information is greater than {@link #getExpiration()}.
	 * @param query The query for requesting a value from the cache.
	...
	/**
	 * Performs any operations that need to be done when cached information is discarded (for example, if the cached information is stale). This version does
	 * nothing.
