...
/**A list that allows quick lookup of {@link IDed} objects.
This list will give unpredictable results if multiple objects with the same ID are added to the list.
This list does not support <code>null</code> values.
...
public class DecoratorIDedMappedList<K, E extends IDed<K>> extends ListDecorator<E> implements MappedList<K, E>	//TODO finish this class if needed
{
	...
	private final Map<K, E> map;
		...
		protected Map<K, E> getMap() {return map;}

	/**Decorates the given list by mapping its values.
	Any values present in the map will be removed.
