	...
	*/
	...
	public void testFixedLengthInputStream() throws IOException
	{
		...
		do
		{
			...
			InputStreams.copy(inputStream, output);
			assertEquals(length, output.toByteArray().length);
			assertTrue("HTTP chunked output stream did not correctly write data.", Arrays.equals(Arrays.copyOfRange(testData, total, total+length), output.toByteArray()));
			total+=length;
		}
