...
/**
 * An abstract collection map that decorates an existing map. Child classes must implements {@link #createCollection()} and return the appropriate type of
 * collection.
...
public abstract class AbstractDecoratorCollectionMap<K, V, C extends Collection<V>> extends MapDecorator<K, C> implements CollectionMap<K, V, C> {
	...
	/**
	 * Removes the first occurrence of the given value from the collection of values, if any, associated with the key. If all items from the collection are
