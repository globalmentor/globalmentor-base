 ...
 * <p>
 * This class maintains weak references to event listeners so that they may be collected by the garbage collector when they are no longer in ordinary use. TODO
 * update comment when weak reference option is added
 * </p>
 * <p>
 * A class is used as a key to a set of event listeners, all of which must be instances of that class or a subclass. Generics are used to ensure that only
 * instances of the class or subclasses are keyed to a particular class. If the event listener type itself is generic, using classes should ensure that all
 * listeners keyed to a particular class are indeed generic subclasses of the class, as Java 5 <code>Class&lt;<var>T</var>&gt;</code> objects do not keep track
 * of the generic type of <var>T</var>.
 * </p>
 * <p>
 * This class uses thread-safe access methods. Returned listener iterators are "snapshots" of currently registered listeners, so may be accessed even though
 * other threads (or even the event listener itself) may add and/or remove listeners.
 * </p>
 * <p>
 * Example:
 * </p>
 ...
 * </p>
 * <p>
 * This class was inspired by <code>javax.swing.EventListenerList</code> 1.33 12/03/01 by Georges Saab, Hans Muller, and James Gosling.
 * </p>
...
public class EventListenerManager //TODO fix to not use WeakHashSet, which isn't a good idea, as anonymous listeners may not be referenced after being added to this class
{
	...
	/**
	 * Removes a listener from the manager as associated with the given key. If the listener is not associated with the key, no action is taken. If all listeners
