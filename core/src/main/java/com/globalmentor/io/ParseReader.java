...
public class ParseReader extends ProcessingBufferedReader //TODO clean up and undeprecate
{
	...
	/**
	 * Returns the name of this reader. If there is no name, and there is a source object, the source object in string form is returned. This function is
	...
	public String getName() {
		if(Name != null) //if we have a name
			...
			//if we don't have a name or a source object
	...
	/**
	 * The index of the last time the line and character indexes were updated. Since The position (line and character) is that of the last character read, this
	...
	/**
	 * Sets the index of the last time the line and character indexes were updated. This is usually set to the current read index after synchronizing the
	 * position.
	 ...
	 * @see ProcessingBufferedReader#getReadIndex
	 */
	protected void setLastPositionIndex(final int lastPositionIndex) {
	...
	/**
	 * Constructor to create a <code>ParseReader</code> from another reader, along with several characters that have already been read.
	...
	/**
	 * Initializes relevant buffer indexes when, for example, the buffer is first created. The parent version of the method is first called and then the last
	 * position index is set.
	 */
	...
	/**
	 * Updates the line and character indexes and sets the last position index to one character before the given index, which is usually the read index. This
	 * method counts the line breaks between the last position index and the read index -1 and updates the line and character indexes appropriately. This method
	 * is called by <code>getLineIndex()</code>, <code>getCharIndex()</code>, and <code>moveBuffer()</code>.
		...
		if(updateIndex - 1 > LastPositionIndex) { //if the update index is more the last place we updated our position
			...
			while(LastPositionIndex < updateIndex - 1) { //while the last position read is under one minus the update index
				if(buffer[LastPositionIndex] == '\n') { //if this was a new line character
					...
					//if this isn't a new line character
	...
	/**
	 * Peeks at the next character to be read, and throws an exception if there are no more characters to read. Each successive peek will return a successive
	 * character. This function is reset with every call to read(). Since every peek reads a successive character, resetPeek() should be called if it must be
	 * ensured that the next character peeked is the next character to be read. This function really reads a character, but all characters peeked will be unread
	...
	/**
	 * Peeks the specified number of characters, but does not throw an exception if the end of the file is reached. This function is reset with every call to
	 * read().
	 * @param len The number of characters to peek.
	 * @return A string containing the characters peeked.
	 * @throws IOException Thrown when a general I/O error occurs.
	...
	/**
	 * Checks to see if the string of characters waiting to be read matches any that are expected. If several strings match, the index of the first match will be
	 * returned. It is not required that peeking be reset before calling this function, but doing so will ensure that any error messages will correctly reflect
	 * the beginning search position. TODO perhaps we could fix this Performance will be increased if strings are arranged in the array from shortest to longest.
	 * TODO do this automatically, later To have this function not throw an exception of none of the strings are found, pass an empty string as the last argument
	 * in expectedStrings.
	...
	/**
	 * Reads a string of characters and makes sure they match a string in a given list, returning the index of the matched string. If several strings match, the
	 * index of the first match will be returned. Resets peeking. Performance will be increased if strings are arranged in the array from shortest to longest.
	 * TODO do this automatically, later To have this function not throw an exception of none of the strings are found, pass an empty string as the last argument
	 * in expectedStrings.
	 * @param expectedStrings An array of strings to expect.
	 * @return The index of the string which is waiting to be read.
	...
	/**
	 * Skips every character that matches the skip characters, and returns the number of characters skipped. Throws an exception if the end of the file is
	...
	/**
	 * Skips every character that matches the skip characters, and returns the number of characters skipped. No exception is thrown if the end of the file is
	 * reached. Resets peeking.
	 * @param skipChars The characters which should be skipped.
	 * @throws IOException Thrown when an i/o error occurs.
	 * @return The number of characters skipped.
	 */
	...
	/**
	 * Reads characters until a particular character is reached or the end of the file is reached. If a delimiter character was reached, it will be the next
	 * character read. No exception is thrown if the end of the file is reached.
	 * @param delimiterChar The character which indicates reading should stop.
	 * @throws IOException Thrown when an i/o error occurs.
	...
	/**
	 * Reads characters until one of the characters in delimiterCharString is reached. The delimiter character reached will be the next character read. Resets
	...
	/**
	 * Reads characters until the given string is reached. The first character of the delimiter string reached will be the next character read. Resets peeking.
	...
	/**
	 * Reads characters up to the given string and skips the given string. Resets peeking. Note that this function has a limitation of the largest integer for the
	...
	/**
	 * Reads characters until one of the characters in delimiterCharString is reached, or the end of the file is reached. If a delimiter character was reached, it
	 * will be the next character read. No exception is thrown if the end of the file is reached. Resets peeking. Note that this function has a limitation of the
	 * largest integer for the number of characters returned.
	 * @param delimiterCharString A list of characters which indicate reading should stop.
	 * @throws IOException Thrown when an i/o error occurs.
	 * @return The characters up to but not including the delimiter character reached or the end of the file.
	 */
		...
		while(!isEnd()) { //if we're not at the end of the file
			...
			while(checkIndex < getFetchBufferIndex()) { //look at each character until the end of the data in this buffer
				if(delimiterCharString.indexOf(buffer[checkIndex]) != -1) { //if this character is one we're looking for
					...
					//if we haven't found a delimiter character
					++checkIndex; //look at the next character
			}
	...
	/**
	 * Reads text from an input stream that that has certain beginning and ending delimiter characters, and returns the string <em>including</em> the delimiters.
	 * @param startDelimiter The character to expect at the first of the characters.
	 * @param endDelimiter The character to expect at the end of the characters.
	 * @throws IOException Thrown when an i/o error occurs.
	 * @throws ParseUnexpectedDataException Thrown when an unexpected character is found.
	...
	/**
	 * Reads text from an input stream that that has certain beginning and ending delimiters, and returns the string <em>including</em> the delimiters.
	 * @param startDelimiter The string to expect at the first of the characters.
	 * @param endDelimiter The string to expect at the end of the characters.
	 * @throws IOException Thrown when an i/o error occurs.
	 * @throws ParseUnexpectedDataException Thrown when an unexpected character is found.
