...
/**
 * Various methods that manipulate {@link StringBuffer} objects. These methods are fast relative to their {@link Strings} counterparts, because the
 * {@link StringBuffer} objects on which they operate can be modified within the object instead of forcing a new object to be created. These methods furthermore
 * modify the specified {@link StringBuffer} object rather than returning a new object.
...
public class StringBuffers {
	...
	public static void collapse(final StringBuffer stringBuffer, final Characters collapseChars, final String replaceString) {
		...
		//TODO del Log.trace("checking character: "+(int)stringBuffer.charAt(nextIndex)+" "+stringBuffer.charAt(nextIndex)); //TODO del
			...
			//TODO del Log.trace("found collapse character at index: ", nextIndex);  //TODO del
				...
				while(removeEnd < stringBuffer.length() && collapseChars.contains(stringBuffer.charAt(removeEnd)))
					//while we still have more characters, and the characters we're finding are characters to remove
					++removeEnd;
	...
	/**
	 * Searches a string buffer and returns the first index of any character <em>not</em> in the specified string, starting from the beginning.
	...
	/**
	 * Searches a string buffer and returns the first index of any character <em>not</em> in the specified string, starting at <code>fromIndex</code>.
	 * @param stringBuffer The string buffer to be searched.
	 * @param notCharString The string of characters to check.
	 ...
	 * @param replacementStrings An array of strings to replace the characters appearing at the same indexes as those in <code>matchChars</code>.
	 * @return The number of replacements made.
	 */
	 ...
	 */
	public static StringBuffer replaceRuns(final StringBuffer stringBuffer, final char matchChar, final int minRunLength, final int maxRunLength,
	...
	/**
	 * Removes several characters at the specified index and replaces them with the given character. This duplicates <code>StringBuffer.replace</code>
	...
	public static StringBuffer trim(final StringBuffer stringBuffer, final String delimiters) {
		...
		for(beginIndex = 0; beginIndex < length && delimiters.indexOf(stringBuffer.charAt(beginIndex)) >= 0; ++beginIndex)
			; //find the first non-delimiter in the string buffer
		for(endIndex = length; endIndex > beginIndex && delimiters.indexOf(stringBuffer.charAt(endIndex - 1)) >= 0; --endIndex)
	...
	//TODO del System.out.println("trimming: "+stringBuffer);  //TODO del
		int endIndex;
		final int length = stringBuffer.length(); //get the length of the original string
		for(endIndex = length; endIndex > 0 && delimiters.indexOf(stringBuffer.charAt(endIndex - 1)) >= 0; --endIndex)
			; //find the last non-delimiter in the string buffer
		if(endIndex < length) //if we should trim the end
			stringBuffer.delete(endIndex, length); //remove the end of the buffer
		return stringBuffer; //return the resulting string buffer
