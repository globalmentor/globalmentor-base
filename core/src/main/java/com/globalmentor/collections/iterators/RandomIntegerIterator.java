 ...
 * can optionally be returned. This iterator can keep track of the numbers used and only return unused integers.
...
public class RandomIntegerIterator implements Iterator<Integer> { //TODO maybe later make this generic for Number subclasses 
	...
	/**
	 * The value that has been retrieved and has passed the filter and is waiting to be returned, or <code>null</code> if there is no primed next object.
	 */
	protected Integer primedNext;

	/**
	 * Shuffle constructor. Constructs an iterator that returns numbers between zero (inclusive) and the given maximum value (exclusive). Values returned will be
	 * 0&le;<var>value</var>&lt;<code>maxValue</code>, and will not be repeated. All numbers will be returned.
	...
	/**
	 * Shuffle constructor with random number generator. Constructs an iterator that returns numbers between zero (inclusive) and the given maximum value
	 * (exclusive). Values returned will be 0&le;<var>value</var>&lt;<code>maxValue</code>, and will not be repeated. All numbers will be returned.
	...
	/**
	 * Constructs an iterator that returns numbers between zero (inclusive) and the given maximum value (exclusive). Values returned will be
	 * 0&le;<var>value</var>&lt;<code>maxValue</code>, and will not be repeated.
	 * @param maxValue The upper bound, exclusive, of the values to return.
	 * @param maxCount The number of integers to return, or -1 if an unlimited number of integers should be returned.
	 * @throws IllegalArgumentException Thrown if the given maximum count is greater than the allowed range.
	 */
	 ...
	 * @param exclude <code>true</code> if the value should be excluded from future calls to <code>next()</code>, else <code>false</code> if it should be possible
	 *          (but not guaranteed) that the value will be returned from future calls to <code>next()</code>.
	 */
	...
	public void setExcluded(final Integer integer, final boolean exclude) {
		if(exclude) //if we should include this value
			...
			//if we shouldn't exclude this value
	...
	/**
	 * Retrieves the next object and stores it locally to return from the next call to <code>next()</code>. If an object is already primed, no action occurs.
