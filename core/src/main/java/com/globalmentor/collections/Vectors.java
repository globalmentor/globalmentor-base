...
public class Vectors {
	...
	/**
	 * Sorts a vector containing {@link Comparable} objects using a quicksort algorithm. Modified from the QSortAlgorithm.java 1.3 program by James Gosling
	...
	private static <E> void quickSort(final Vector<E> vector, final int low, final int high) {
		...
		if(high > low) { //if the original high index is lower than the original low index
			...
			while(loIndex <= hiIndex) { //loop through the array until indices cross
				while((loIndex < high) && (((Comparable<E>)vector.elementAt(loIndex)).compareTo(midElement) < 0))
				...
				while((hiIndex > low) && (((Comparable<E>)vector.elementAt(hiIndex)).compareTo(midElement) > 0))
