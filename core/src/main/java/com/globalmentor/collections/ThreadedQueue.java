...
/**
 * A queue of objects that, once an object is added to a queue, will process the queue contents in a separate thread. Once all objects in the queue have been
 * processed, the separate thread will block until more objects are added to the queue.
 * <p>
 * The queue does not start processing automatically unless constructed with the autostart constructor. Otherwise, it must first be started by calling its
 * {@link #start()} method.
 * </p>
 * <p>
 * A subclass may override <code>process(Object)</code>, or use the <code>ObjectProcessor</code> constructor, specifying itself or another class as the object
 * processor.
 * </p>
...
public class ThreadedQueue implements Runnable, ObjectProcessor {
	...
	/**
	 * The list of objects. This list is not synchronized, so every method that accesses it must syncrhonize on the list itself.
	 */
	private final LinkedList queueList = new LinkedList();

	/**
	 * The thread used for processing the objects. Also used as an indication of whether the queue is started or stopped.
	 */
	 ...
	 * @param newObjectProcessor The object responsible for processing objects as they are added to the queue, or <code>null</code> if this class should be used
	 *          as the object processor.
	 */
	 ...
	 * @param autoStart <code>true</code> if the processing thread should start automatically.
	 */
	...
	public ThreadedQueue(final ObjectProcessor newObjectProcessor, final boolean autoStart) {
		//TODO maybe assert the object processor isn't null
	...
	/**
	 * Causes this queue to begin processing its contents, if any, in a separate thread of execution. If the queue is already started, no action is performed.
	 * //TODO del @throws IllegalThreadStateException Thrown if the processing thread was //TODO del already started.
	...
	/**
	 * Stops the thread that is processing this queue's contents. This method blocks until the processing thread has processed all remaining objects in the queue.
	 * If the queue is already stopped, no action occurs.
	 */
	...
	public void run() { //TODO will this thread be garbage collected when the ThreadedQueue is garbage collected?
		while(processThread != null) { //process objects until the processing thread is set to null, meaning that we should stop
			...
			//TODO del				while(queueList.size()==0) //if there are no objects in the queue
				{
					try {
						queueList.wait(); //wait for another object to be added to the queue
					} catch(InterruptedException interruptedException) {
					} //if we're interrupted while waiting, ignore it and keep waiting, unless we have objects in the queue or the thread should end
				}
			}
			try {
	...
	/**
	 * Processes any objects in the queue. Called from the processing thread when objects are added to the queue. It is recommended that the
	 * <code>process(Object)</code> method be overridden instead of this method.
	 */
	public void process() {
	...
	/**
	 * Processes a single object in the queue. Called from the processing thread when objects are added to the queue. The default constructor specifies this class
	 * as the object processor, so child classes can override this method to process each object if a separate object processor is not specified.
