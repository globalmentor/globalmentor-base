...
//G***del if not needed	protected final static String VALUE_OF="valueOf";
...
//G***del if not needed	protected final static Class NO_PARAMETERS=new Class[]{};
...
/*G***del
	public XMLStorage()
	{
		storedVariableList=getStoredVariables();	//G***fix
...
*/
	...
	//G***comment
	public static void retrieve(final Object object, final Document document, final StoredVariable[] storedVariableArray) throws XMLStorageException
	{
	...
	//G***comment
	public static void store(final Object object, final Document document, final StoredVariable[] storedVariableArray) throws XMLStorageException
	{
		for(int i=0; i<storedVariableArray.length; ++i)
		{
			final StoredVariable storedVariable=storedVariableArray[i];
			store(storedVariable.name, storedVariable.type, object, document, storedVariable.locationPath);	//G***testing
		}
	}

	//G***comment
/*G***fix or delete
	public void retrieve(final Object object, final Document document) throws XMLStorageException
	{
...
*/
	...
	@exception XMLStorageException Thrown if <code>object</code> has no
	...
	*/
	public static Object retrieve(final String name, Class type, final Object object, final Document document, final String locationPath, final Object defaultValue) throws XMLStorageException
	{
		try
		{
	...
	//G***del when works		Object returnObject=null;	//this is the object we'll return
			...
			if(nodeList.getLength()>0)	//if we retrieved at least one value from the document
			{
				...
				try
				{
//G***fix					Method convertMethod;	//we'll store here the method to use to convert the string to the object
					...
					else	//if it's not a primitive type, we'll try to use a string constructor
					{
						...
						{
							throw new XMLStorageException(e.toString()+": "+type.getName()+'.'+type.getName()+"("+String.class.getName()+")");	//create and throw a storage exception
						}
/*G***del when works
...
//G***del					final Object convertedValue=convertMethod.invoke(null, new Object[]{stringValue});	//convert the string to the appropriate type of object using valueOf()
						//G***check the NullPointerException here to make sure the convertMethod is static
				}
...
/*G***del when works
				...
				{
					throw new XMLStorageException(e.toString()+": "+type.getName()+'.'+VALUE_OF+"("+String.class.getName()+")");	//create and throw a storage exception
				}
*/
			}
			else	//if we couldn't find a value in the document
				convertedValue=defaultValue;	//we'll use the default value
//G***del				return defaultValue;	//return the default value
			try
			{
			...
			{
				throw new XMLStorageException("Location path "+locationPath+" not found in XML document.");	//show that we couldn't find the value in the document G***is this the best message to return?
			}
			return convertedValue;	//return the value we set
		}
		catch(NoSuchMethodException e)	//if the setname() function was not found
		{
			throw new XMLStorageException(e.toString()+": "+object.getClass().getName()+'.'+SET+name+"("+type.getName()+")");	//create and throw a storage exception
		}
		catch(IllegalAccessException e)	//if the setname() or valueOf() cannot be accessed
		{
			throw new XMLStorageException(e.toString());	//create and throw a storage exception
		}
	...
	@exception XMLStorageException Thrown if <code>object</code> has no
		...
		<code>valueOf(String)</code> method. This is thrown in place of such exceptions
		as java.lang.reflect.InvocationTargetException and java.lang.NoSuchMethodException;
//G***have all these comments been converted?
	*/
	public static void store(final String name, Class type, final Object object, final Document document, final String locationPath) throws XMLStorageException
	{
		try
		{
...
*/
				...
				throw new XMLStorageException("Unable to store value in XML document at XPath location path \""+locationPath+"\".");	//throw an error showing tha twe can't fin the location in the document
		}
		catch(NoSuchMethodException e)	//if the setname() function was not found
		{
			throw new XMLStorageException(e.toString()+": "+object.getClass().getName()+'.'+GET+name+"()");	//create and throw a storage exception
		}
		catch(IllegalAccessException e)	//if the setname() or valueOf() cannot be accessed
		{
			throw new XMLStorageException(e.toString());	//create and throw a storage exception
	...
	*/
	...
	{
		//G***check object for null, and throw an invalid argument exception if it is
	...
	*/
	protected static void store(final Object object, final Element element)
	{
		...
		{
Debug.trace("ready to store object: ", object.getClass().getName());  //G***del
Debug.trace("using element: ", element.getNodeName());  //G***del
			...
			{
	Debug.trace("object is array with length: ", Array.getLength(object));			  //G***del
...
/*G***del when works
							...
							final String storageName=JavaUtilities.getVariableName(propertyName); //get the name of the attribute or element
	Debug.trace("invoking method: ", method.getName()); //G***del
*/
		/*G***fix
		...
		*/
								...
								else if(!returnType.isInterface())  //if the return type isn't an interface
								{
									...
									{
										final Object valueObject=method.invoke(object, NO_ARGUMENTS);	//call the object's getXXX() method to get the value to store
										if(!valueObject.equals(object)) //if the property doesn't point back to the object (such as Point.getLocation()), making a circular reference G***maybe have some sort of better checking for circular references
		...
		/*G***fix
							...
							{
								final Object childObject=method.invoke(object, new Object[]{});	//invoke the get method and retrieve a reference to its return value G***let's not do this unless we know the return type is instanceof XMLStorable
...
*/
	...
	*/
	...
	{
		final XMLProcessor xmlProcessor=new XMLProcessor(); //create a new XML processor G***use JAXP or something
	...
	*/
	...
	@exception FileNotFoundException Thrown if the specified file is invalid.
	@exception IOException Thrown if there is an error writing to the file.
	*/  //G***add a boolean return value which has the status of the actual file move
...
Debug.trace("Storing in file: ", file); //G***del
	...
	*/
	...
	{
Debug.trace("ready to retrieve object: ", objectClass.getName());  //G***del
Debug.trace("using element: ", element.getNodeName());  //G***del
...
Debug.trace("object is array");
			...
			final List childElementList=XMLUtilities.getNodesByNameNS(element, Node.ELEMENT_NODE, null, "*", false);  //G***use a constant here
//G***del when works			final NodeList childElementList=element.getElementsByTagNameNS(null, "*"); //get all the child elements
...
//G***del when works		  for(int i=0; i<childElementCount; ++i)  //construct an object for each child element
			{
				try
				{
...
//G***del					componentType.newInstance();  //create a new instance of the object
				}
				...
				{
					Debug.warn(e);  //warn of the error but try to construct the rest of the object G***should we do something more severe here?
				}
		...
		{
Debug.trace("Ready to create object: ", objectClass.getName()); //G***del
		  object=createObject(element, objectClass);  //create an object for this element
Debug.trace("Created object: ", object);  //G***del
//G***del when works			object=objectClass.newInstance(); //create a new instance of the class, using its default constructor
...
*/
					...
					//G***somehow warn if the object was not stored
				}
			}
...
Debug.trace("has one parameter of type: ", parameterType.getName());
	...
	*/
	...
	{
//G***del		final Object object;  //we know we'll be able to at least create the object
	...
	*/
	protected static Object createObject(final Element element, final Class type, final Class[] constructorParameterTypes, final String[] constructorPropertyNames) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException
	{
		...
		//G***maybe make sure the parameter type array and name array are the same length
...
//G***del					try
					{
//G***del Debug.trace("Getting property "+constructorPropertyNames[parameterIndex]+" for type "+parameterTypes[parameterIndex]);  //G***del
...
//G***del Debug.trace("Got property: ", arguments[parameterIndex]); //G***del
					}
/*G***del
...
//G***del when works				if(parameterIndex<0)  //if we successfully created all arguments
					return constructor.newInstance(arguments); //invoke the constructor with the arguments we created
/*G***del when works
		  }
		}
		throw new InstantiationException("Could not find an appropriate constructor for "+type.getName()+" for the properties "+constructorPropertyNames+".");  //show that we couldn't find an appropriate constructor G***do we want to throw an error, or just warn here?
*/
	}
	...
	*/
	...
	{
Debug.trace("Using default constructor to create instance of ", type.getName());  //G***del
Debug.trace("Looking for method: ", methodName);  //G***del
...
*/
		...
		{
Debug.trace("Ready to initialize object: ", object);  //G***del
			method.invoke(object, arguments);  //invoke the method of the object using the arguments we created
Debug.trace("Initialized object: ", object);  //G***del
...
Debug.trace("Found method: ", method);  //G***del
...
*/
	...
	/**Retrieves a property value from the specified element with the appropriate
		...
		for property values. G***fix for child elements, if needed
	@param element The element that holds the information about the object.
	...
	*/
	protected static Object getProperty(final Element element, final String propertyName, final Class propertyType) throws InstantiationException
	{
		...
		//G***check here for child elements: else
...
//G***del		final Class objectType=object.getClass(); //find out what type of object has properties to be stored
		...
		if(objectType.equals(java.awt.Point.class)) //if they are storing a Point
		{
			if(propertyName.equals("X") || propertyName.equals("Y"))  //if the X or Y values are being stored
				return true;  //store them, even though they don't have corresonding setXXX() methods
//G***del			if(propertyName.equals("Location")) //the recursive "location
		}
	...
	/**The encapsulation of the information needed to store and retrieve a variable
		...
		(it is a primitive value, for example), an <code>XMLStorageException</code>
		will be thrown.
	*/
