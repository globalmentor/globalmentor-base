...
public class QTIResponseLIDAnswerPanel extends JPanel
{
  ...
  JTextField scoreTextField = new JTextField();  //G***testing
...
/*G***fix
		...
		answerListModel.addElement(new JRadioButton("Test radio button 1"));  //G***testing
		...
		answerList.setUI(new ToggleListUI()); //G***testing
		answerList.setCellRenderer(new CheckBoxListCellRenderer());  //G***testing
*/
...
*/
	...
	{
//G***del		final ResponseProcessing responseProcessing;  //we'll create response processing if there are selections
		if(answerList.getSelectedIndex()>=0) //if there is at least one answer selected
		{
			...
			responseProcessing.getOutcomes().getDecVarList().add(scoreDecVar);  //add the score variable declaration G***check all this
			final ResponseCondition responseCondition=new ResponseCondition("correct"); //create a new response condition section G***i18n
			...
			setVar.setAction(SetVar.SET_ACTION);  //show that the variable should be set
				//G***check the value of the setvar, and decide what we'll do about all this string stuff---shouldn't our data model be more typesafe?
	...
	*/  //G***eventually we may want to pass the entire presentation, as it may contain multiple responseLIDs
	public void setResponseProcessing(final ResponseLID responseLID, final ResponseProcessing responseProcessing)
	{
		this.responseLID=responseLID; //store the logical ID response for later so that we can correctly set the respident when retrieving the response processing G***if we later allow the user to change the responseLID ident, this will need to be changed
...
//G***del				final RenderChoice renderChoice=(RenderChoice)render; //cast the render to a render choice
		...
		//G***check the setvar condition to make sure this is for the score
					if(responseCondition.getSetVarList().size()>0) //if there are variables being set in response to these conditions
					{
						final SetVar setVar=(SetVar)responseCondition.getSetVarList().get(0); //get the first setvar G***change to iterate through the setvars to find the one for SCORE
					...
					while(conditionVarIterator.hasNext())  //while there are more conditions sets G***clarify with QTI WG why there could be multiple condition sets
					{
						//G***fix; we only use one conditionvar group
	...
	Debug.trace("found condition variable with response label ID: ", responseLabelID);  //G***del
							...
							if(responseLabel!=null) //if this condition matches a response label
							{
								...
								answerList.addSelectionInterval(responseLabelIndex, responseLabelIndex);  //G***testing
//G***fix								  answerList.setSelectedValue(responseLabel, true); //G***testing
							}
						}
					}
				}
			...
			else  //if there is no response processing
			{
				...
				scoreTextField.setText("1");  //default to a score of one G***maybe use a constant here
			}
		}
	}
}